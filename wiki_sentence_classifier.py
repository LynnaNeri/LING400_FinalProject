...
for file in os.listdir(): #https://www.geeksforgeeks.org/how-to-iterate-over-files-in-directory-using-python/
    if not file.startswith('.') and not file.startswith('Icon'):
        ...
        text_sentences = nltk.tokenize.sent_tokenize(text) #https://stackoverflow.com/a/31505798
        for contents in text_sentences:
            mental.append((page, 0, contents))
    
    count = 0
    for sentence in mental:
        if sentence[0] == page:
            count += 1
    mental_sent_count.append((page, count))
...
for file in os.listdir():
    if not file.startswith('.') and not file.startswith('Icon'):
        ...
        text_sentences = nltk.tokenize.sent_tokenize(text)
        for contents in text_sentences:
            physical.append((page, 1, contents))
    
    count = 0
    for sentence in physical:
        if sentence[0] == page:
            count += 1
    physical_sent_count.append((page, count))

sentence_counts = []
for file in mental_sent_count:
    sentence_counts.append(file[1])
for file in physical_sent_count:
    sentence_counts.append(file[1])
print(min(sentence_counts))
...
for file in os.listdir():
    ...
    for sentence in mental:
        if sentence[0] == file:
            if count < 30:
                cut_mental.append(sentence)
                count += 1
    count = 0
    topic = []
    for sentence in cut_mental:
        if sentence[0] == file:
            count += 1
            topic.append(sentence[1:])
...
for file in os.listdir():
    ...
    for sentence in physical:
        if sentence[0] == file:
            if count < 30:
                cut_physical.append(sentence)
                count += 1
    count = 0
    topic = []
    for sentence in cut_physical:
        if sentence[0] == file:
            count += 1
            topic.append(sentence[1:])
...
for topic, content in data_train_prel:
    for label, sentence in content:
        data_train.append((topic, label, sentence))
data_val = []
for topic, content in data_val_prel:
    for label, sentence in content:
        data_val.append((topic, label, sentence))
data_test = []
for topic, content in data_test_prel:
    for label, sentence in content:
        data_test.append((topic, label, sentence))
...
dump((train_page, train_label, train_content), '../../sentence_train.joblib')
...
dump((val_page, val_label, val_content), '../../sentence_val.joblib')
...
dump((test_page, test_label, test_content), '../../sentence_test.joblib')
...
#Save models.
dump((vectorize, nb, svm, rf, lg), '../../sentence_models.joblib')